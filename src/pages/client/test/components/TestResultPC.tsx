...
const TestResultPC = ({ onNailClick }: TestResultPCProps) => {
  ...
  const nails = recommendedNails.slice(0, 6);
    ...
    <main className="w-full max-w-3xl mx-auto bg-white border border-stone-200 rounded-2xl shadow-sm px-10 py-12 flex flex-col items-center">
          <div className="w-full">
            ...
            <div className="mt-3 flex justify-center">
              <span className="inline-flex items-center justify-center rounded-full bg-[#FFEFE9] px-4 py-1.5 text-[14px] font-bold text-[#FF826E]">
                {isEnglish && result.styleTag_en ? result.styleTag_en : result.styleTag}
              </span>
            </div>

            {/* 추천 이유 */}
            <div className="mx-auto mb-10 mt-6 max-w-xl rounded-2xl border border-[#FFE4DB] bg-[#FFF7F3] p-6">
            ...
            {/* 메인 추천 6장 (3열 2줄) */}
            <div className="grid grid-cols-3 gap-4 w-full mt-8 pb-10">
              {isLoading ? (
                Array.from({ length: 6 }, (_, index) => (
                  <div key={`diagnosis-pc-skel-${index}`} className="flex w-full flex-col gap-2" aria-hidden>
                    <div className="aspect-[3/4] w-full animate-pulse overflow-hidden rounded-2xl bg-gray-200" />
                    <div className="mx-auto h-3 w-3/4 animate-pulse rounded bg-gray-200" />
                  </div>
                ))
              ) : isError || nails.length === 0 ? (
                <p className="col-span-3 py-12 text-center text-[15px] font-medium text-gray-500">
