...
const styleItems = [
  { label: "심플 네일", keyword: "심플" },
  { label: "화려한 네일", keyword: "화려한" },
  { label: "프렌치 네일", keyword: "프렌치" },
  { label: "드로잉 네일", keyword: "드로잉" },
];

const patternTrends = [
  { title: "마블 네일", keyword: "마블" },
  { title: "그라데이션 네일", keyword: "그라데이션" },
  { title: "체크 네일", keyword: "체크" },
  { title: "트위드 네일", keyword: "트위드" },
];

const textureTrends = [
  { title: "시럽 네일", keyword: "시럽" },
  { title: "마그넷 네일", keyword: "마그넷" },
];

const partTrends = [
  { title: "스톤/큐빅 네일", keyword: "스톤 큐빅" },
  { title: "리본 네일", keyword: "리본" },
  { title: "진주 네일", keyword: "진주" },
];
            ...
            {colorNailChips.map((chip) => (
              ...
              >
                <div className="w-14 h-14 sm:w-16 sm:h-16 relative mx-auto flex items-center justify-center rounded-full bg-gray-50 shadow-sm overflow-hidden">
                  <img src={chip.src} alt={chip.label} className="w-full h-full object-cover" onError={handleImageError} />
                </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            {styleItems.map((item) => {
              ...
              >
                {imageUrl ? (
                  <img src={imageUrl} alt={item.label} className="w-full aspect-[4/5] object-cover object-center rounded-2xl shadow-sm" onError={handleImageError} />
                ) : (
          ...
          <div className="-mx-5 flex gap-3 overflow-x-auto px-5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {CUSTOM_THEME_PILLS.map((label) => (
              <button
                key={label}
                type="button"
                onClick={() => goTheme(extractPureThemeKeyword(label))}
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-4">
            {patternTrends.map((item) => {
              ...
              >
                {imageUrl ? (
                  <img src={imageUrl} alt={item.title} className="w-full aspect-[4/5] object-cover rounded-2xl shadow-sm" onError={handleImageError} />
                ) : (
