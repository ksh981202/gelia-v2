          ...
          <div className="mb-4 rounded-2xl border border-stone-200/60 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-end gap-3">
              <h1 className="text-2xl font-bold tracking-tight text-stone-800 md:text-3xl">
                {isEnglish ? "✨ Premium Nail Collection" : "✨ 프리미엄 네일 컬렉션"}
              </h1>
              <span className="mb-1 text-base font-semibold tracking-wide text-stone-600 md:text-lg">
                {isEnglish ? (
                  <>
                    <span className="font-bold text-orange-600">{countLabel}</span> nail designs
                  </>
                ) : (
                  <>
            ...
            <div className="border-l-4 border-stone-200 py-1 pl-4">
              <p className="text-base font-medium text-stone-600">
                {isEnglish
                  ? "👑 Design collection prepared for your clients."
                  : "👑 원장님이 고객님을 위해 준비한 디자인 컬렉션입니다."}
              </p>
              <p className="mt-1 text-base text-stone-600">
                {isEnglish
                  ? "💎 Take a look and find the perfect style."
          ...
          <div className="mb-4 rounded-2xl border border-stone-200/60 bg-white p-6 shadow-sm">
            <div className="flex items-end gap-3">
              <h1 className="text-2xl font-bold text-stone-800">
                {isEnglish ? "💅 All Design Gallery" : "💅 전체 디자인 갤러리"}
              </h1>
              <span className="mb-1 text-sm font-medium text-stone-500">
                {galleryStats.hasActiveFilters ? (
                  isEnglish ? (
                    <>
                      <span className="font-semibold text-[#5C4A3A]">{countLabel}</span> search results
                    </>
                  ) : (
                    <>
                  ...
                  <>
                    <span className="font-semibold text-[#5C4A3A]">{countLabel}</span> premium designs
                  </>
                ) : (
                  <>
            ...
            <div className="mt-4 text-sm leading-relaxed text-stone-600">
              <p className="mb-3">
                {isEnglish ? (
                  <>
                    Tap the{" "}
                    <strong className="font-semibold text-stone-800">[☑️ checkbox]</strong> on each
                    photo to add designs and start a personalized consultation.
                  </>
                ) : (
                  <>
                    사진 우측 상단의{" "}
              ...
              <ul className="space-y-2">
                <li>
                  {isEnglish ? (
                    <>
                      <strong className="font-semibold text-stone-800">⭐ My Collection:</strong> Save
                      frequently used designs for quick access
                    </>
                  ) : (
                    <>
                ...
                <li>
                  {isEnglish ? (
                    <>
                      <strong className="font-semibold text-stone-800">📋 Proposals:</strong> Curate
                      designs for each client and create a dedicated proposal for 1:1 consultations.
                    </>
                  ) : (
                    <>
